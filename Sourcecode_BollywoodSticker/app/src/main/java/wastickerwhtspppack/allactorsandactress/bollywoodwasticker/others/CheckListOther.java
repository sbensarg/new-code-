...
public class CheckListOther {
  ...
  private static final int STICKER_FILE_SIZE_LIMIT_KB = 100;
    ...
    public static void verifyStickerPackValidity(@NonNull Context context, @NonNull PackOfSickerParcelable stickerPack) throws IllegalStateException {
        ...
        try {
            final byte[] bytes = LoaderPackages.fetchStickerAsset(stickerPack.identifier, stickerPack.trayImageFile, context.getContentResolver());
        ...
        final List<StickerParcelable> stickers = stickerPack.getStickers();
        ...
        for (final StickerParcelable sticker : stickers) {
            validateSticker(context, stickerPack.identifier, sticker);
        }
    }

    private static void validateSticker(@NonNull Context context, @NonNull final String identifier, @NonNull final StickerParcelable sticker) throws IllegalStateException {
        if (sticker.emojis.size() > EMOJI_LIMIT) {
            throw new IllegalStateException("emoji count exceed limit, sticker pack identifier:" + identifier + ", filename:" + sticker.st_imgFileName);
        }
        if (TextUtils.isEmpty(sticker.st_imgFileName)) {
            throw new IllegalStateException("no file path for sticker, sticker pack identifier:" + identifier);
        }
        validateStickerFile(context, identifier, sticker.st_imgFileName);
    }
    ...
    private static void validateStickerFile(@NonNull Context context, @NonNull String identifier, @NonNull final String fileName) throws IllegalStateException {
        try {
            final byte[] bytes = LoaderPackages.fetchStickerAsset(identifier, fileName, context.getContentResolver());
    ...
    private static boolean isValidWebsiteUrl(String websiteUrl) throws IllegalStateException {
        try {
            ...
            Log.e("CheckListOther", "url: " + websiteUrl + " is malformed");
    ...
    private static boolean isURLInCorrectDomain(String urlString, String domain) throws IllegalStateException {
        try {
            ...
            Log.e("CheckListOther", "url: " + urlString + " is malformed");
